...
abstract class BasePagingSource<T : TMDbItem>(private val context: Context) : PagingSource<Int, T>() {
    ...
    protected abstract suspend fun fetchWeekItems(page: Int): List<T>
    ...
    suspend fun loadWeekItems(params: LoadParams<Int>): LoadResult<Int, T> {
        val page = params.key ?: STARTING_PAGE_INDEX
        return try {
            val response = fetchWeekItems(page)
            LoadResult.Page(
                data = response,
                prevKey = if (page == STARTING_PAGE_INDEX) null else page - 1,
                nextKey = if (response.isEmpty()) null else page + 1
